...
for input_size in [10]:
    ...
    with lstm_graph.as_default():
        ...
        # CNN
        # last_output = tf.expand_dims(last_output, 2)
        #
        # filters1 = tf.Variable(tf.truncated_normal([5, 1, 128]))
        # cnn_output = tf.nn.conv1d(last_output, filters=filters1, stride=1, padding="SAME")
        #
    ...
    with tf.Session(graph=lstm_graph) as sess:
        ...
        for epoch in range(config.max_epoch):
            ...
            # batch_index = 0
            for batch_X, batch_Y in utils.generate_one_epoch(train_data, train_labels, config.batch_size, config.num_steps):
                train_data_feed = {
                    inputs: batch_X,
                    targets: batch_Y,
