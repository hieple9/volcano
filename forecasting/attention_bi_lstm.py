...
for kk in [10]:
    ...
    with lstm_graph.as_default():
        ...
        # Attention mechanism
        attention_weight = tf.Variable(tf.truncated_normal([config.lstm_size*2, config.output_size]))
        attention_bias = tf.Variable(tf.constant(0.1, shape=[config.output_size]))
        val = tf.reshape(val, [-1, config.lstm_size*2])
        ...
        attention_contribution = tf.tile(attention_contribution, [config.lstm_size*2])
        attention_contribution = tf.transpose(tf.reshape(attention_contribution, [config.lstm_size*2, -1]))
        val = tf.reshape(val, [-1, config.lstm_size*2])
        hidden_contribution = attention_contribution * val
        hidden_contribution = tf.reshape(hidden_contribution, [-1, (config.num_steps)*config.lstm_size*2])
        ...
        weight2 = tf.Variable(tf.truncated_normal([config.num_steps*config.lstm_size*2, config.output_size]))
