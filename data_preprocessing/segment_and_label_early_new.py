...
sliding_window = 50
label_interval = 10
for num_of_data_point in ts_len:
    for pre in ahead:
        ...
        for year in years:
            ...
            new_strain_data = pd.read_csv('../data/new_strain/data_%s.csv' % year)
            ...
            new_strain_len = len(new_strain_data)
            print "new_strain_len", new_strain_len
            ...
            new_strain_starting_time = new_strain_data.time[0]
            new_strain_ending_time = new_strain_data.time[new_strain_len - 1]
            print new_strain_starting_time
            print new_strain_ending_time

            # starting_time = strain_starting_time if strain_starting_time > energy_starting_time else energy_starting_time
            starting_time = max(strain_starting_time, energy_starting_time, new_strain_starting_time)
            ...
            ending_time = min(strain_ending_time, energy_ending_time, new_strain_ending_time)
            ...
            maximum_amplitudes = []
            new_radial_strains = []
            new_tangential_strains = []

            pre_radial_strains = []
            pre_tangential_strains = []
            pre_energies = []
            pre_maximum_amplitudes = []
            pre_new_radial_strains = []
            pre_new_tangential_strains = []
            ...
            while head_time + np.timedelta64(num_of_data_point + pre, 'm') < ending_time:
                ...
                if len(strain_index) != 1 or len(energy_index) != 1 or len(new_strain_index) != 1:
                    ...
                    print "new_strain_index", new_strain_index
                    print head_time
                strain_index = strain_index[0]
                energy_index = energy_index[0]
                new_strain_index = new_strain_index[0]
                ...
                new_strain = strain_data.iloc[new_strain_index:new_strain_index + num_of_data_point]

                pre_strain = strain_data.iloc[strain_index + pre:strain_index + num_of_data_point + pre]
                ...
                if not is_missing_point(strain.time.values) and \
                    ...
                    if pre_explosion_position == 0:
                        ...
                        label_counter[pre_explosion_position] += 1
